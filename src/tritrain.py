...
transfo_train = Compose([
    ...
    RandomVerticalFlip(),
    RandomRotation(90, interpolation=torchvision.transforms.InterpolationMode.BILINEAR, expand=True),
...
path = "data"
train_dataset = BreslowUlcerationRelapseDataset(train_dataframe, os.path.join(path, "images"), transforms=transfo_train)
val_dataset = BreslowUlcerationRelapseDataset(val_dataframe, os.path.join(path, "images"), None)

train_loader = DataLoader(train_dataset, batch_size=8, num_workers=30, shuffle=True, pin_memory=True)
val_loader = DataLoader(val_dataset, batch_size=1, num_workers=30, shuffle=False, pin_memory=True)
...
for epoch in range(epochs):
    ...
    print("Train loss: {:.4f} - Train loss breslow: {:.4f} - Train loss ulceration: {:.4f} - Train loss relapse: {:.4f}".format(train_loss, train_loss_b, train_loss_u, train_loss_r))
    print("Train acc breslow: {:.4f} - Train acc ulceration: {:.4f} - Train acc relapse: {:.4f}".format(train_acc_b, train_acc_u, train_acc_r))
    ...
    running_loss_r = 0.
    with torch.no_grad():
        for i, item in tqdm.tqdm(enumerate(val_loader), desc="Validation...", total=len(val_loader)):
            slides = item["slide"].to(device)
            breslow = item["breslow"].to(device)
            ulceration = item["ulceration"].to(device)
            relapse = item["relapse"].to(device)
    ...
    print("Val loss: {:.4f} - Val loss breslow: {:.4f} - Val loss ulceration: {:.4f} - Val loss relapse: {:.4f}".format(val_loss, val_loss_b, val_loss_u, val_loss_r))
