...
######## Create dataset :
make_dataframe_with_latent_space(model_path=root+"/models/tritrain.pth",
...
def run_model(config, train_dataframe, val_dataframe, features_names):
    #### Train
    model = ImageTabularModel(len(features_names),config["model_type"],
                            dropout=config["dropout"], relapse_only=config["relapse_only"])
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=7, verbose=True, factor=0.2)
    best_loss = np.Inf
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    train_dataset = ImageTabular(train_dataframe, features_names, aug_embedding=None, aug_tabular=None)
