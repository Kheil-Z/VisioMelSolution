...
import torch
from torchvision.transforms import RandomRotation, Compose, RandomHorizontalFlip, RandomVerticalFlip, RandomCrop, ColorJitter, GaussianBlur, ToTensor, Normalize
...
def create_test_df(device, args):
    ...
    test_dataframe = test_dataframe.drop(columns=['tif_cksum', 'tif_size',
                                                    'us_tif_url', 'eu_tif_url','as_tif_url',
                                                    'ulceration', 'breslow', 'resolution'],
                                                    errors='ignore')
    ...
    body_site_mapping= {'nan': -1, 'trunk': 0, 'arm': 1, 'seat': 2, 'head': 3, 'neck': 3, 'head/neck': 3,
                        'face': 4, 'trunc': 0, 'leg': 5, 'forearm': 6, 'upper limb': 7, 'shoulder': 7, 'upper limb/shoulder': 7,
                        'lower limb': 8, 'hip': 8, 'lower limb/hip': 8, 'hand': 9, 'toe': 10,
                        'foot': 9, 'nail': 9, 'hand/foot/nail': 9, 'thigh': 11, 'sole': 12, 'finger': 13, 'scalp': 14}  
    maps = {"sex": sex_mapping,
...
def get_image(path, transforms = Compose([ToTensor(),Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])])):
...
def main(args):
    ...
    # Get preds :
    preds_dict = predict(device, test_dataframe, features_names, args.model_type, dropout, args.relapse_only, args.model_path, age_trick = args.age_trick_bool)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="models/after_finetune.pth", help='Path to trained model used as classifier.')
    parser.add_argument('--tritrain_path',type=str, default="models/tritrain.pth", help='Path to trained resNet model used as feature extractor.')
    parser.add_argument('--model_type', type=str, default="FC", help='Tpe of model ("FC" or "CNN")')
    parser.add_argument('--relapse_only', type=bool, default=False, help='Describes model outputs (see model class)')

    parser.add_argument('--data_path', type=str, default="data/", help='Path to data, must contain metada.csv and tiff format images.')
    parser.add_argument('--age_trick_bool', type=bool, default=True, help='use age trick boolean at inference')
