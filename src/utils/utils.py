...
def train_epoch(device,
    ...
    running_loss_relapse_bce = 0. # REMOVE

    
    for i, (item) in tqdm(enumerate(loader), desc=f"{train_type}...", total=len(loader), disable=not verbose):
        if patches :
            img_embedding, patch_embedding, tabular, target_relapse, target_breslow, target_ulceration = item
        ...
        with torch.no_grad():
            running_loss_relapse_bce += torch.nn.BCELoss()(relapse_pred.squeeze(1), target_relapse).item()* img_embedding.size(0) # REMOVE
    ...
    if verbose:
        print("Loss: {:.4f} - Loss breslow: {:.4f} - Loss ulceration: {:.4f} - Loss relapse: {:.4f}".format(train_loss, train_loss_b, train_loss_u, train_loss_r))
        print("Acc breslow: {:.4f} - Acc ulceration: {:.4f} - Acc relapse: {:.4f}".format(train_acc_b, train_acc_u, train_acc_r))
    
    # Update min loss relapse found:
    if (train_loss_r < min_loss_r):
        min_loss_r = train_loss_r
    return train_loss, labels_r, labels_u, labels_b, predictions_r, predictions_u, predictions_b, min_loss_r, train_loss_r
