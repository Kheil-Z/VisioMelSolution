...
def make_dataframe_with_latent_space(model_path="tritrain.pth",
    ...
    # Append elements : 
    latent_df_train,latent_df_val = pd.DataFrame.from_dict(logits_dict_train), pd.DataFrame.from_dict(logits_dict_val)
    ...
    # Remove columns :
    train_dataframe = train_dataframe.drop(columns=['tif_cksum', 'tif_size', 'us_tif_url', 'eu_tif_url','as_tif_url', 'weights', 'resolution'],errors='ignore')
    val_dataframe = val_dataframe.drop(columns=['tif_cksum', 'tif_size', 'us_tif_url', 'eu_tif_url','as_tif_url', 'resolution'], errors='ignore')
